...
class PersistentAthleteArray: NSObject, NSCoding {
    ...
    init?(names: [Athlete], fileName: String) {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        guard let names = aDecoder.decodeObject(forKey: PropertyKey.stringNames) as? [Athlete] else {
    ...
    func restoreString() -> [Athlete]?  {
        let tmp = NSKeyedUnarchiver.unarchiveObject(withFile: ArchiveURL.path) as? [Athlete]
        return tmp
    }
}
